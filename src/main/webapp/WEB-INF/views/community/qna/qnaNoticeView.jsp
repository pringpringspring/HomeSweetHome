...
첨부파일:
				...
				<img alt="첨부파일" src="<%=request.getContextPath() %>/images/file.jpg" width=16px>
				<a href="<%= request.getContextPath() %>/board/fileDownload?no=<%= attach.getNo() %>"><%= attach.getOriginalFilename() %></a>
				<br>
				<img src="<%=request.getContextPath() %>/upload/qna/<%=attach.getRenamedFilename() %>" width=500px><br>
			...
			<% 
				for (QnaNoticeComment nc : comments){ 
					...
					boolean canDelete = loginMember != null 
							&& (loginMember.getMemberId().equals(nc.getMemberId()) 
									|| loginMember.getMemberRole() == MemberRole.A);
...
<form action="<%=request.getContextPath()%>/qna/qnaNoticeCommentDelete" name="boardCommentDelFrm" method="POST">
...
<% if(canEdit){ %>
<form action="<%= request.getContextPath() %>/qna/qnaNoticeDelete" name="boardDeleteFrm" method="POST">
